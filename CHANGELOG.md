...
### Added
- [`Strategy10`](src/main/java/com/moneymaker/strategy/Strategy10.java) (`stratergy_id = 10`): Strategy 9 tuned for intraday — expiry gate off, candle threshold 0.35 (changeset 050 seeds its `strategy_defaults` row from strategy 9's), last entry bar 13:15 via the new `Strategy8.entryCutoffMinutesBeforeCloseSignal()` hook. Strategy 9 now reads its `strategy_defaults` row by `getId()` so subclasses get their own. Pinned by `Strategy10RulesTest`; S31.
- [`Strategy9`](src/main/java/com/moneymaker/strategy/Strategy9.java) (`stratergy_id = 9`): Strategy 8 plus three configurable entry gates from the indicator / volume study — signal candle not closing on its low, no near-ATM option-volume spike on the signal bar (summed over the cached 15-minute legs within ±200 of the session-open ATM vs the trailing 25-bar median), and a minimum days-to-expiry. Changeset 049 adds `strategy_defaults.max_volume_surge` / `min_candle_close_position` / `min_days_to_expiry` (null = off) and seeds strategy 9 from strategy 8's block (2.00 / 0.250 / 2). Pinned by `Strategy9RulesTest`; S30.
