...
| [`046_seed_strategy6_defaults.xml`](../src/main/resources/db/changelog/046_seed_strategy6_defaults.xml) | Seeds the `strategy_defaults` row for [`Strategy6`](STRATEGIES.md#strategy-6--strategy-2-with-higher-timeframe-confirmation-an-entry-cut-off-and-a-stop-loss-lock) by **copying strategy 1's block** (`opposite_side = FALSE`), so the two share one generated config as `"1,…,6"` — the shape its edge was measured on. Idempotent; no-op without a strategy-1 row. Does **not** tag any rule: strategy 6 trades nothing until the operator inserts its `sma_downtrend_rule_strategy` rows (SQL in the changeset comment and in STRATEGIES.md). |
| [`047_seed_strategy7_defaults.xml`](../src/main/resources/db/changelog/047_seed_strategy7_defaults.xml) | The same for [`Strategy7`](STRATEGIES.md#strategy-7--strategy-6-with-the-first-hour-regime-gate): strategy 1's block copied for strategy 7, no rule tagged. |
